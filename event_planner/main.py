...
def service_components():
    ...
    if st.button("세부사항 입력"):
        ...
        for category in selected_categories:
            ...
            component['status'] = st.radio(f"{category} 진행 상황", status_options, index=status_options.index(event_data.get(f'status_{category}', status_options[0])))
            ...
            component['items'] = st.multiselect(f"{category} 항목 선택", item_options.get(category, []), default=event_data.get(f'items_{category}', []))

            for item in component['items']:
                component[f'{item}_quantity'] = st.number_input(f"{item} 수량", min_value=0, value=event_data.get(f'{item}_quantity', 0))
                component[f'{item}_unit'] = st.text_input(f"{item} 단위", value=event_data.get(f'{item}_unit', '개'))
