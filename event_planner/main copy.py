...
def handle_general_info(event_data: Dict[str, Any]) -> None:
    ...
    def update_event_name():
        event_data['event_name'] = st.session_state.event_name_input

    event_data['event_name'] = st.text_input("프로젝트명", value=event_data.get('event_name', ''), key="event_name_input", on_change=update_event_name)
    event_data['client_name'] = st.text_input("클라이언트명", value=event_data.get('client_name', ''), key="client_name_basic", on_change=lambda: event_data.update({'client_name': st.session_state.client_name_basic}))
    event_data['manager_name'] = st.text_input("담당 PM", value=event_data.get('manager_name', ''), key="manager_name_basic", on_change=lambda: event_data.update({'manager_name': st.session_state.manager_name_basic}))
    event_data['manager_email'] = st.text_input("담당 PM 이메일", value=event_data.get('manager_email', ''), key="manager_email_basic", on_change=lambda: event_data.update({'manager_email': st.session_state.manager_email_basic}))

    event_data['manager_position'] = render_option_menu(
        "담당자 직급",
        options=["선임", "책임", "수석"],
        key="manager_position",
        on_change=lambda: event_data.update({'manager_position': st.session_state.manager_position})
    )
    ...
    manager_contact = st.text_input(
        ...
        on_change=lambda: event_data.update({'manager_contact': format_phone_number(''.join(filter(str.isdigit, st.session_state.manager_contact_basic)))})
    )
    event_data['manager_contact'] = manager_contact

def render_option_menu(label: str, options: List[str], key: str, on_change=None) -> str:
...
def handle_event_type(event_data: Dict[str, Any]) -> None:
    event_data['event_type'] = render_option_menu(
        ...
        "event_type",
        on_change=lambda: event_data.update({'event_type': st.session_state.event_type})
    )
    event_data['contract_type'] = render_option_menu(
        "용역 종류",
        event_options.CONTRACT_TYPES,
        "contract_type",
        on_change=lambda: event_data.update({'contract_type': st.session_state.contract_type})
    )
...
def handle_budget_info(event_data: Dict[str, Any]) -> None:
    ...
    event_data['contract_status'] = render_option_menu(
        ...
        "contract_status",
        on_change=lambda: event_data.update({'contract_status': st.session_state.contract_status})
    )
    ...
    vat_included = option_menu(
        ...
        key="vat_included",
        on_change=lambda: event_data.update({'vat_included': st.session_state.vat_included == vat_options[0]})
    )
    ...
    event_data['contract_amount'] = st.number_input(
        ...
        key="contract_amount",
        format="%d",
        on_change=lambda: event_data.update({'contract_amount': st.session_state.contract_amount})
    )
    ...
    if event_data['contract_status'] == "추가 예정":
        event_data['additional_amount'] = st.number_input(
            ...
            format="%d",
            on_change=lambda: event_data.update({'additional_amount': st.session_state.additional_amount})
        )
    ...
    event_data['expected_profit_percentage'] = st.number_input(
        ...
        on_change=lambda: event_data.update({'expected_profit_percentage': st.session_state.expected_profit_percentage})
    )
...
def handle_video_production(event_data: Dict[str, Any]) -> None:
    ...
    with col1:
        start_date = st.date_input("과업 시작일", 
                                   ...
                                   on_change=lambda: event_data.update({'start_date': st.session_state.video_start_date}))
    with col2:
        ...
        end_date = st.date_input("과업 종료일",
                                 ...
                                 key="video_end_date",
                                 on_change=lambda: event_data.update({'end_date': st.session_state.video_end_date}))
...
def handle_offline_event(event_data: Dict[str, Any]) -> None:
    ...
    with col1:
        start_date = st.date_input("시작 날짜",
                                   value=event_data.get('start_date', date.today()),
                                   key="start_date",
                                   on_change=lambda: event_data.update({'start_date': st.session_state.start_date}))
    ...
    with col2:
        end_date = st.date_input("종료 날짜",
                                 ...
                                 min_value=start_date,
                                 key="end_date",
                                 on_change=lambda: event_data.update({'end_date': st.session_state.end_date}))
    ...
    with col3:
        event_data['setup_start'] = render_option_menu("셋업 시작일", config['SETUP_OPTIONS'], "setup_start", on_change=lambda: event_data.update({'setup_start': st.session_state.setup_start}))

    with col4:
        event_data['teardown'] = render_option_menu("철수 마감일", config['TEARDOWN_OPTIONS'], "teardown", on_change=lambda: event_data.update({'teardown': st.session_state.teardown}))
...
def venue_info() -> None:
    ...
    if event_data['event_type'] == "온라인 콘텐츠":
        ...
        event_data['venue_status'] = render_option_menu(
            ...
            "venue_status",
            on_change=lambda: event_data.update({'venue_status': st.session_state.venue_status})
        )
        ...
        event_data['venue_type'] = render_option_menu(
            ...
            venue_type_options,
            "venue_type",
            on_change=lambda: event_data.update({'venue_type': st.session_state.venue_type})
        )
        ...
        if event_data['venue_type'] != "온라인":
            event_data['scale'] = st.number_input(
                ...
                on_change=lambda: event_data.update({'scale': st.session_state.scale_input_venue})
            )
...
def handle_online_content_location(event_data: Dict[str, Any]) -> None:
    location_needed = render_option_menu(
        ...
        ["필요", "불필요"],
        "location_needed",
        on_change=lambda: event_data.update({'location_needed': st.session_state.location_needed})
    )
    ...
    if location_needed == "필요":
        location_type = render_option_menu(
            ...
            "location_type",
            on_change=lambda: event_data.update({'location_type': st.session_state.location_type})
        )
        ...
        if location_type == "프로덕션이 알아서 구해오기":
            location_preference = render_option_menu(
                ...
                ["실내", "실외", "혼합"],
                "location_preference",
                on_change=lambda: event_data.update({'location_preference': st.session_state.location_preference})
            )

            if location_preference == "실내":
                event_data['indoor_location_description'] = st.text_area("어떤 느낌의 장소인지 작성해주세요.", key="indoor_location_description", on_change=lambda: event_data.update({'indoor_location_description': st.session_state.indoor_location_description}))
                if len(event_data['indoor_location_description']) < 50:
                    st.error("장소 설명은 최소 50자 이상 작성해야 합니다.")
            elif location_preference == "실외":
                event_data['outdoor_location_description'] = st.text_area("어떤 느낌의 장소인지 작성해주세요.", key="outdoor_location_description", on_change=lambda: event_data.update({'outdoor_location_description': st.session_state.outdoor_location_description}))
            ...
            event_data['location_type'] = render_option_menu(
                ...
                on_change=lambda: event_data.update({'location_type': st.session_state.direct_location_type})
            )
            event_data['location_name'] = st.text_input("장소명", key="location_name", on_change=lambda: event_data.update({'location_name': st.session_state.location_name}))
            event_data['location_address'] = st.text_input("주소", key="location_address", on_change=lambda: event_data.update({'location_address': st.session_state.location_address}))
            event_data['location_status'] = render_option_menu(
                "확정의 정도를 선택해주세요.",
                event_options.STATUS_OPTIONS,
                "location_status",
                on_change=lambda: event_data.update({'location_status': st.session_state.location_status})
            )
...
def handle_unknown_venue_status(event_data: Dict[str, Any]) -> None:
    ...
    event_data['desired_region'] = st.selectbox(
        ...
        on_change=lambda: event_data.update({'desired_region': st.session_state.desired_region_selectbox})
    )

    event_data['specific_location'] = st.text_input("세부 지역 (선택사항)", value=event_data.get('specific_location', ''), key="specific_location", on_change=lambda: event_data.update({'specific_location': st.session_state.specific_location}))
    event_data['desired_capacity'] = st.number_input("희망하는 수용 인원", min_value=0, value=int(event_data.get('desired_capacity', 0)), key="desired_capacity", on_change=lambda: event_data.update({'desired_capacity': st.session_state.desired_capacity}))
...
def handle_known_venue_status(event_data: Dict[str, Any]) -> None:
    ...
    for i, venue in enumerate(event_data['venues']):
        ...
        col1, col2 = st.columns(2)
        with col1:
            venue['name'] = st.text_input("장소명", value=venue.get('name', ''), key=f"venue_name_{i}", on_change=lambda: event_data['venues'][i].update({'name': st.session_state[f"venue_name_{i}"]}))
        with col2:
            venue['address'] = st.text_input("주소", value=venue.get('address', ''), key=f"venue_address_{i}", on_change=lambda: event_data['venues'][i].update({'address': st.session_state[f"venue_address_{i}"]}))
...
def handle_venue_facilities(event_data: Dict[str, Any]) -> None:
    if event_data['venue_type'] in ["실내", "혼합"]:
        ...
        event_data['facilities'] = st.multiselect("행사장 자체 보유 시설", facility_options, default=event_data.get('facilities', []), key="facilities", on_change=lambda: event_data.update({'facilities': st.session_state.facilities}))

        if "기타" in event_data['facilities']:
            event_data['other_facilities'] = st.text_input("기타 시설 입력", key="other_facility_input", on_change=lambda: event_data.update({'other_facilities': st.session_state.other_facility_input}))

def handle_venue_budget(event_data: Dict[str, Any]) -> None:
    event_data['venue_budget'] = st.number_input("장소 대관 비용 예산 (원)", min_value=0, value=int(event_data.get('venue_budget', 0)), key="venue_budget", format="%d", on_change=lambda: event_data.update({'venue_budget': st.session_state.venue_budget}))
    ...
    for i, category in enumerate(categories):
        with [col1, col2, col3, col4][i % 4]:
            if st.checkbox(
                ...
                on_change=lambda: selected_categories.append(st.session_state[f"category_{category}_{i}"])
            ):
...
def handle_category(category: str, event_data: Dict[str, Any]) -> None:
    ...
    component['status'] = render_option_menu(
        ...
        event_options.STATUS_OPTIONS,
        f"{category}_status",
        on_change=lambda: component.update({'status': st.session_state[f"{category}_status"]})
    )
    ...
    component['items'] = st.multiselect(
        ...
        on_change=lambda: component.update({'items': st.session_state[f"{category}_items"]})
    )

    component['budget'] = st.number_input(f"{category} 예산 (원)", min_value=0, value=component.get('budget', 0), key=f"{category}_budget", on_change=lambda: component.update({'budget': st.session_state[f"{category}_budget"]}))
    ...
    shooting_date_status = render_option_menu(
        ...
        on_change=lambda: component.update({'shooting_date_status': st.session_state[f"{category}_shooting_date_status"]})
    )
    ...
    with col1:
        component['shooting_start_date'] = st.date_input(
            ...
            on_change=lambda: component.update({'shooting_start_date': st.session_state[f"{category}_shooting_start_date"]})
        )
    with col2:
        component['shooting_end_date'] = st.date_input(
            ...
            on_change=lambda: component.update({'shooting_end_date': st.session_state[f"{category}_shooting_end_date"]})
        )
    ...
    for idx, delivery in enumerate(component['delivery_dates']):
        ...
        delivery['status'] = render_option_menu(
            ...
            on_change=lambda: delivery.update({'status': st.session_state[f"{category}_delivery_status_{idx}"]})
        )
        ...
        if delivery['status'] == "정해짐":
            delivery_type = render_option_menu(
                ...
                on_change=lambda: delivery.update({'type': st.session_state[f"{category}_delivery_type_{idx}"]})
            )
            ...
            if delivery_type == "기간":
                ...
                with col1:
                    delivery['start_date'] = st.date_input(
                        ...
                        on_change=lambda: delivery.update({'start_date': st.session_state[f"{category}_delivery_start_date_{idx}"]})
                    )
                with col2:
                    delivery['end_date'] = st.date_input(
                        ...
                        on_change=lambda: delivery.update({'end_date': st.session_state[f"{category}_delivery_end_date_{idx}"]})
                    )
            else:
                delivery['date'] = st.date_input(
                    ...
                    on_change=lambda: delivery.update({'date': st.session_state[f"{category}_delivery_date_{idx}"]})
                )
        else:
            delivery['date'] = None
        ...
        for item in component['items']:
            quantity = st.number_input(
                ...
                on_change=lambda: delivery['items'].update({item: st.session_state[f"{category}_delivery_item_{idx}_{item}"]})
            )
            if quantity > 0:
    ...
    if category == "미디어":
        ...
        for i, link in enumerate(component['reference_links']):
            component['reference_links'][i] = st.text_input(
                f"레퍼런스 링크 {i+1} (필수)",
                value=link,
                key=f"{category}_reference_link_{i}",
                on_change=lambda: component['reference_links'].update({i: st.session_state[f"{category}_reference_link_{i}"]})
            )
    ...
    component['cooperation_status'] = render_option_menu(
        ...
        on_change=lambda: component.update({'cooperation_status': st.session_state[f"{category}_cooperation_status"]})
    )
    ...
    for item in component['items']:
        if item == "기타":
            component['other_details'] = st.text_area(f"{category} 기타 세부사항", value=component.get('other_details', ''), key=f"{category}_other_details", on_change=lambda: component.update({'other_details': st.session_state[f"{category}_other_details"]}))
        else:
...
def handle_media_component(component: Dict[str, Any], category: str) -> None:
    ...
    if shooting_date_status == "정해짐":
        component['shooting_date'] = st.date_input(
            ...
            on_change=lambda: component.update({'shooting_date': st.session_state[f"{category}_shooting_date"]})
        )
    else:
        col1, col2 = st.columns(2)
        with col1:
...
def handle_preferred_vendor(component: Dict[str, Any], category: str) -> None:
    component['vendor_reason'] = render_option_menu(
        ...
        on_change=lambda: component.update({'vendor_reason': st.session_state[f"{category}_vendor_reason"]})
    )
    component['vendor_name'] = st.text_input("선호 업체 상호명", value=component.get('vendor_name', ''), key=f"{category}_vendor_name", on_change=lambda: component.update({'vendor_name': st.session_state[f"{category}_vendor_name"]}))
    component['vendor_contact'] = st.text_input("선호 업체 연락처", value=component.get('vendor_contact', ''), key=f"{category}_vendor_contact", on_change=lambda: component.update({'vendor_contact': st.session_state[f"{category}_vendor_contact"]}))
    component['vendor_manager'] = st.text_input("선호 업체 담당자명", value=component.get('vendor_manager', ''), key=f"{category}_vendor_manager", on_change=lambda: component.update({'vendor_manager': st.session_state[f"{category}_vendor_manager"]}))
...
def handle_item_details(item: str, component: Dict[str, Any]) -> None:
    ...
    with col1:
        component[quantity_key] = st.number_input(f"{item} 수량", min_value=0, value=component.get(quantity_key, 0), key=quantity_key, on_change=lambda: component.update({quantity_key: st.session_state[quantity_key]}))

    with col2:
        component[unit_key] = st.text_input(f"{item} 단위", value=component.get(unit_key, '개'), key=unit_key, on_change=lambda: component.update({unit_key: st.session_state[unit_key]}))

    with col3:
        component[duration_key] = st.number_input(f"{item} 기간", min_value=0, value=component.get(duration_key, 0), key=duration_key, on_change=lambda: component.update({duration_key: st.session_state[duration_key]}))

    with col4:
        component[duration_unit_key] = st.text_input(f"{item} 기간 단위", value=component.get(duration_unit_key, '개월'), key=duration_unit_key, on_change=lambda: component.update({duration_unit_key: st.session_state[duration_unit_key]}))

    component[details_key] = st.text_area(f"{item} 세부사항", value=component.get(details_key, ''), key=details_key, on_change=lambda: component.update({details_key: st.session_state[details_key]}))
